...
4. ตั้งค่าข้อมูลในแถบ **ตั้งค่า (Setup)**:
    - **เมตริก (Metric):** ลากฟิลด์ `ร้อยละยอดเบิกจ่าย` ที่เพิ่งสร้างขึ้น ไปวางทับ `งบประมาณ` (หรือฟิลด์ใด ๆ ก็ตามที่ระบบเลือกให้โดยอัตโนมัติ)

5. คลิกที่รูปดินสอหน้าชื่อเมตริก `ร้อยละยอดเบิกจ่าย` เพื่อเปลี่ยนหน่วย

    ![ตั้งค่าเมตริก](img/ws3-s4-5.png){ width="800" }

    ที่หัวข้อ **ประเภทข้อมูล (Data type)** เลือก **ตัวเลข (Numeric) > เปอร์เซ็นต์ (Percent)**

    ![เปลี่ยนประเภทข้อมูล](img/ws3-s4-6.png){ width="400" }

6. เปิดตัวเลือก **แสดงแกน (Show axis)** ในแถบย่อย **รูปแบบ (Style)** จากนั้นตั้ง **ค่าต่ำสุดของแกน (Minimum value)** และ **ค่าสูงสุดของแกน (Maximum value)** เป็น `0` และ `1` ตามลำดับ
